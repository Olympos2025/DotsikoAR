...
### Σημαντική διευκρίνιση για αυτοματοποίηση

Το παρόν περιβάλλον ανάπτυξης δεν έχει τη δυνατότητα να συνδεθεί απευθείας με τον λογαριασμό GitHub
σας, ακόμη κι αν διαθέτει repository URL ή προσωπικό access token. Για να δημιουργηθεί το αρχικό
repository και να ενεργοποιηθεί το GitHub Pages workflow πρέπει να εκτελεστεί μια διαδικασία από τη
δική σας πλευρά ή από υποδομή που ελέγχετε. Έχει συμπεριληφθεί το script
[`scripts/bootstrap_github_pages.sh`](scripts/bootstrap_github_pages.sh) το οποίο αυτοματοποιεί την
δημιουργία του repository μέσω του GitHub CLI (`gh`), ρυθμίζει το `origin`, κάνει push στον `main`
και ενεργοποιεί το workflow. Εκτελέστε το σε δικό σας περιβάλλον με πρόσβαση στον λογαριασμό σας.

Μόλις ολοκληρωθεί επιτυχώς μία φορά, κάθε μελλοντικό push στο `main` θα αναλαμβάνεται αυτόματα από το
υπάρχον workflow και θα ενημερώνει το URL GitHub Pages.
